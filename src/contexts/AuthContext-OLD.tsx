...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  useEffect(() => {
    ...
    const getInitialSession = async () => {
      try {
        ...
        if (error) {
          ...
          // Call backend for auto-recovered session
          if (refreshedSession?.user) {
            await handleAutoRecoveredSession(refreshedSession.user);
          }
        } else {
          setSession(session);
          setUser(session?.user ?? null);
          ...
          // Call backend for auto-recovered session
          if (session?.user) {
            await handleAutoRecoveredSession(session.user);
          }
        }
      } catch (err) {
    ...
    // Handle auto-recovered session like manual login
    const handleAutoRecoveredSession = async (user: any) => {
      try {
        console.log("Auto-recovery: Fetching user role for", user.email);
        
        // เรียก backend API เหมือน manual login
        const backendResponse = await authAPI.login({ 
          email: user.email, 
          password: "auto-recovery" // ส่ง flag บอกว่าเป็น auto-recovery
        });
        
        console.log("Auto-recovery backend response:", backendResponse);

        // เก็บ role ใน sessionStorage เหมือน manual login
        if (backendResponse && backendResponse.role) {
          console.log("Auto-recovery: Storing user role:", backendResponse.role);
          sessionStorage.setItem("userRole", backendResponse.role);
          setUserRole(backendResponse.role);
        }
      } catch (backendError) {
        console.warn("Auto-recovery backend call failed:", backendError);
        
        // Fallback: ใช้ข้อมูลจาก sessionStorage ที่เหลือ (ถ้ามี)
        const storedRole = sessionStorage.getItem("userRole");
        if (storedRole) {
          console.log("Auto-recovery: Using stored role:", storedRole);
          setUserRole(storedRole);
        }
      }
    };
